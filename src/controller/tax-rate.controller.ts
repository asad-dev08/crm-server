...
import { TaxRate } from "../interface/tax-rate";
import { connection } from "../config/mysql.config";
import { QUERY } from "../query/tax-rate.query";
...
export const getDataWithPagination = async (
  ...
  try {
    ...
    const result: ResultSet = await pool.query(
      `SELECT *  FROM tax_rate order by created_at desc LIMIT ? OFFSET ? `,
...
export const getTaxRates = async (
  ...
  try {
    const pool = await connection();
    const result: ResultSet = await pool.query(QUERY.SELECT_TAX_RATES);

    return res
      .status(Code.OK)
      .send(
        new HttpResponse(Code.OK, Status.OK, "Tax Rate retrived", result[0])
      );
  } catch (error: unknown) {
    console.log(error);
    return res
...
export const getTaxRate = async (
  ...
  try {
    const pool = await connection();
    const result: ResultSet = await pool.query(QUERY.SELECT_TAX_RATE, [
      req.params.taxRateId,
    ]);

    if ((result[0] as Array<ResultSet>).length > 0) {
      const tax_rate = (result[0] as RowDataPacket[])[0];

      return res.status(Code.OK).send(
        new HttpResponse(Code.OK, Status.OK, "Tax Rate Rtrived", {
          ...tax_rate,
        })
      );
    } else {
...
export const createTaxRate = async (
  ...
  try {
    ...
    const tax_rateModel = {
      ...
      rate: tax_rate.rate,
      is_active: tax_rate.is_active || false,
    ...
    const result: ResultSet = await pool.query(
      generateInsertQuery(tax_rateModel, "tax_rate"),
      [...Object.values(tax_rateModel)]
    );
...
export const updateTaxRate = async (
  ...
  try {
    ...
    if ((result[0] as Array<ResultSet>).length > 0) {
      const updateTaxRate: ResultSet = await pool.query(
        generateUpdateQuery(tax_rateModel, "tax_rate", "id"),
        [...Object.values(tax_rateModel), req.params.taxRateId]
      );
      ...
      return res.status(Code.OK).send(
        new HttpResponse(Code.OK, Status.OK, "TaxRate Updated", {
          ...tax_rate,
          id: req.params.taxRateId,
        })
      );
    } else {
      return res
...
export const deleteTaxRate = async (
  ...
  try {
    ...
    if ((result[0] as Array<ResultSet>).length > 0) {
      const updateTaxRate: ResultSet = await pool.query(QUERY.DELETE_TAX_RATE, [
        [req.params.taxRateId],
      ]);
      await auditLogger.logAction(
        "tax_rate",
      ...
      return res
        .status(Code.OK)
        .send(new HttpResponse(Code.OK, Status.OK, "TaxRate Deleted!"));
    } else {
      return res
        .status(Code.NOT_FOUND)
        .send(
