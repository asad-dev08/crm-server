...
import { QUERY } from "../query/customer-type-query";
...
export const getDataWithPagination = async (
  ...
  try {
    ...
    const result: ResultSet = await pool.query(
      `SELECT *  FROM customer_type order by created_at desc LIMIT ? OFFSET ? `,
...
export const getCustomerTypes = async (
  ...
  try {
    const pool = await connection();
    const result: ResultSet = await pool.query(QUERY.SELECT_CUSTOMER_TYPES);
    ...
    return res
      ...
      .send(
        new HttpResponse(
          Code.OK,
          Status.OK,
          "Customer Billing Addresss retrived",
          result[0]
        )
      );
  } catch (error: unknown) {
...
export const getCustomerType = async (
  ...
  try {
    const pool = await connection();
    const result: ResultSet = await pool.query(QUERY.SELECT_CUSTOMER_TYPE, [
      req.params.typeId,
    ]);
    ...
    if ((result[0] as Array<ResultSet>).length > 0) {
      ...
      return res.status(Code.OK).send(
        new HttpResponse(Code.OK, Status.OK, "CustomerType Rtrived", {
          ...type,
        })
      );
    } else {
...
export const createCustomerType = async (
  ...
  let type: CustomerType = { ...req.body };
  ...
  try {
    ...
    const result: ResultSet = await pool.query(
      generateInsertQuery(typeModel, "customer_type"),
...
export const updateCustomerType = async (
  ...
  try {
    ...
    if ((result[0] as Array<ResultSet>).length > 0) {
      const updateCustomerType: ResultSet = await pool.query(
        generateUpdateQuery(typeModel, "customer_type", "id"),
      ...
      return res.status(Code.OK).send(
        new HttpResponse(Code.OK, Status.OK, "Billing Address Updated", {
          ...type,
...
export const deleteCustomerType = async (
  ...
  try {
    ...
    if ((result[0] as Array<ResultSet>).length > 0) {
      ...
      return res
        .status(Code.OK)
        .send(new HttpResponse(Code.OK, Status.OK, "Billing Address Deleted!"));
    } else {
      return res
        .status(Code.NOT_FOUND)
        .send(
